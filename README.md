...
- **Три режима зондирования**: TWAMP (`TWping`), **TWampy** (свой исполняемый файл, вывод совместим с TWAMP) и системный `ping`; параметры командной строки задаются на задачу. Режим фиксируется в каждом результате и показан во всех таблицах и в отчёте (колонка `Mode`).
...
| `twampy:name` | `TWampy` | исполняемый файл для режима TWampy (запуск как TWamp) |
