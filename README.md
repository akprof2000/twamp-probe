...
| `Probe:MaxParallel` | `64` | **сколько процессов зонда работает одновременно** (пул воркеров) |
...
> **Подбор `Probe:MaxParallel`.** Каждый зонд — отдельный процесс ОС. Слишком большое значение насыщает CPU и замедляет всё, включая ответы HTTP. Ориентир: `4 × число ядер`. На 16-ядерной машине значение 64 обрабатывает 10 000 задач заметно быстрее, чем 200 (см. [Производительность](#производительность)).
...
| Вкладка | Возможности |
|---|---|
| **Статус проб** | связь/ошибки/backoff, версия пробы, число задач и результатов; подтверждение новых проб; CheckIn по адресу |
| **Задачи** | список с фильтром, статус, таймаут, удаление задачи |
| **Загрузка** | шаблоны CSV → файл маршрутизаторов (предпросмотр или создание) → либо готовый CSV задач |
...
Выгрузка из системы инвентаризации (M2000 и т. п.). Берётся **только первое поле** каждой строки вида `ИМЯ|IP:адрес` — состав и порядок остальных колонок не важны. Строка заголовка (`SNODE …`) распознаётся и пропускается автоматически; поддерживаются файлы и без заголовка.

```
231101|IP:10.106.23.33	4	HUAWEI	7	Краснодарский край	…
ADCTO24G|IP:10.23.179.54	4	HUAWEI	7	Краснодарский край	…
```
