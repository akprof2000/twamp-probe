...
- **«удалить»** — удалить пробу. Сначала спросит подтверждение, затем **отдельно** спросит, удалять ли **все её задачи**. Опрос пробы немедленно останавливается; далее срабатывает [авто-очистка](#автоматическая-очистка-при-удалении-пробы).
...
на вкладке «Загрузка» (или через [HTTP API](#http-api)); эта вкладка нужна, чтобы их
...
**① Наборы шаблонов.** Загрузите CSV шаблонов (формат — в [Форматах файлов](#форматы-файлов)).
Каждый файл образует именованный **набор** (имя по умолчанию — имя файла, можно задать своё).
Наборов может быть сколько угодно; ниже — таблица загруженных наборов с числом шаблонов и
кнопкой удаления набора. Повторная загрузка набора с тем же именем обновляет его.
...
сервер в память. Состав колонок — в [Форматах файлов](#4-отчёт-выгрузка-downloadfile).
...
Для интеграций и автоматизации те же операции доступны через [HTTP API](#http-api);
