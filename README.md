...
| **Задачи** | список с фильтром, статус, таймаут и **два статуса последнего запуска**: «Выполнение» — как завершился процесс (завершилась сама / убита по таймауту / не запустилась) и «Результат» — итог приложения (успех «код 0» / ошибка «код N» + текст ошибки); после перезапуска сервера статусы восстанавливаются из БД |
