...
| 🧵 **[Сколько замеров одновременно](docs/parallelism.md)** | **сколько зондов можно запускать сразу, почему один замер стоит процесса и потока, настройка CentOS на максимум** |
...
- **Хранение измерений в ClickHouse**: сервер копит результаты в дисковом буфере и переносит их пачками (по числу строк или по таймауту), после чего удаляет у себя. Если база недоступна — сегменты копятся, а исчерпав буфер, сервер приостанавливает опрос проб, и данные ждут на них (см. [путь результатов](docs/architecture.md#путь-результатов-в-clickhouse)).
...
- **Проба одним бинарником**: ~6 МБ на Go, без .NET и внешних библиотек — работает на CentOS 7+, Rocky, Alma, Ubuntu, Debian и Windows, ставится службой одной командой (см. [Проба](docs/go-probe.md)).
- **Встроенные зонды TWamp и TWampy**: замер идёт прямо в процессе пробы, без запуска внешней утилиты. Такой замер не занимает ни процесса, ни потока ожидания, поэтому предел одновременных замеров на него не распространяется (см. [Сколько замеров одновременно](docs/parallelism.md)). Клиент TWAMP — библиотека [twping-go](https://github.com/akprof2000/twping-go), та же, из которой собрана утилита `twping`. На 50 одновременных замерах: внешний путь — 50 процессов и 340 МБ, встроенный — ни одного процесса и 41 МБ, значения совпадают ([измерения](docs/go-probe.md#встроенные-зонды-против-внешних-утилит)).
...
- **Сервер**: .NET 10 SDK (для сборки) или .NET 10 Runtime (для запуска); вариант
  selfcontained не требует ничего.
...
- Для режима **TWampy** — Python 3.8+ на пробе, либо **встроенный отправитель**
  (`twampy:embedded`), которому python не нужен. Сам twampy вендорен в проекте
  (папка `go-probe/twampy/`, копируется в пакет).
...
Проще всего взять [готовый релиз](https://github.com/akprof2000/twamp-probe/releases)
и поставить службой одной командой — установщики лежат внутри **каждого**
архива, и для сервера, и для пробы:
...
Первый — для Linux (служба systemd, лимиты и настройки ядра), второй — для
Windows (служба, порт в брандмауэре; права администратора запросит сам).
Повторный запуск обновляет версию, не трогая ваш `appsettings.json`.
