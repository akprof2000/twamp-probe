...
| Вкладка | Возможности |
|---|---|
| **Статус проб** | связь/ошибки/backoff, версия пробы, число задач и результатов; кнопка **«задачи»** — живой **составной статус** выполнения на пробе: запустилась ли задача, как завершился процесс (успех «код 0» / ошибка «код N» / не запустился / таймаут), счётчики успехов и ошибок, краткий результат (итоговая строка вывода зонда), следующий запуск; кнопка **«удалить»** — удаление пробы с остановкой опроса (спросит, удалять ли и её задачи); у неопознанных проб — кнопка **«Отклонить»** |
...
собираются **четыре архива для Linux x64** (в каждом — папка приложения целиком) и публикуются в GitHub Release:
...
| `twamp-probe-*-framework.tar.gz` | проба, маленькая | .NET 10 Runtime (ASP.NET Core) |
