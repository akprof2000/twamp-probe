...
| Логи | консоль + файл с ротацией (NLog) | **консоль + файл с ротацией и gzip** (log/slog) |
...
удаляются. Формат строки совпадает с журналом C#-пробы и сервера — все три читаются
одинаково:
