...
| `Probe:MaxParallel` | `0` | **сколько процессов зонда работает одновременно** (пул воркеров). `0` — автоподбор `число ядер × 16` с потолком **1024** и полом 16 (на 16 ядрах — 256). Любое значение `> 0` используется как есть |
...
> (потолок 1024, пол 16). Это разумно для основного случая — зонды это внешние процессы,
