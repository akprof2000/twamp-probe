...
| `Probe:MaxParallel` | `0` | **сколько процессов зонда работает одновременно** (пул воркеров). `0` — автоподбор `число ядер × 64` с потолком **10000** и полом 16 (на 16 ядрах — 1024). Любое значение `> 0` используется как есть |
...
> **Подбор `Probe:MaxParallel`.** По умолчанию `0` — проба сама берёт `число ядер × 64`
...
> - **длинные I/O-замеры TWAMP** (`twping -c 300 -i 1` живёт ~5 минут и почти спит) — параллелизм должен покрывать число одновременно активных задач: при 900 задачах с cron `*/5` одновременно работают все 900 — автоподбор на 16 ядрах как раз даёт 1024;
