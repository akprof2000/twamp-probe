...
| `Probe:MaxParallel` | `0` | **сколько процессов зонда работает одновременно** (пул воркеров). `0` — автоподбор `число ядер × 10` с потолком **10000** и полом 16 (на 16 ядрах — 160). Любое значение `> 0` используется как есть |
...
> **Подбор `Probe:MaxParallel`.** По умолчанию `0` — проба сама берёт `число ядер × 10`
