...
| `Probe:MaxParallel` | `0` | **сколько процессов зонда работает одновременно** (пул воркеров). `0` — автоподбор `число ядер × 256` с потолком **100000** и полом 16 (на 16 ядрах — 4096). Любое значение `> 0` используется как есть |
