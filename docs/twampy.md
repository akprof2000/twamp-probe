...
### Встроенный C#-отправитель (эксперимент)

Режим `sender` портирован на C# и работает **прямо в процессе пробы**, без запуска
внешнего python-процесса. Включается флагом в `appsettings.json`:

```json
"twampy": { "name": "python3", "default": "", "embedded": true }
```

При `embedded: true` задачи режима `TWampy` выполняет
[`TwampySender`](../SPI.TWamp.Probe/Runners/TwampySender.cs): тот же протокол TWAMP-Light,
...
([`TwampySenderTests`](../SPI.Twamp.Tests/TwampySenderTests.cs)): обмен с реальным
рефлектором, разбор серверным парсером, случай полной потери пакетов.
