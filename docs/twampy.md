...
### Встроенный отправитель (эксперимент): C# и Go

Режим `sender` портирован **и на C#, и на Go** и работает **прямо в процессе пробы**,
без запуска внешнего python-процесса. Включается одним флагом в `appsettings.json`
(работает для обеих проб — C# и Go):

```json
"twampy": { "name": "python3", "default": "", "embedded": true }
```

При `embedded: true` задачи режима `TWampy` выполняет встроенный отправитель —
[`TwampySender`](../SPI.TWamp.Probe/Runners/TwampySender.cs) в C#-пробе или
[`twampysender.go`](../go-probe/twampysender.go) в Go-пробе: тот же протокол TWAMP-Light,
те же формулы задержек/джиттера [RFC1889]/потерь и **тот же текст таблицы**, поэтому
серверный `TwampyParser`, отчёты и БД не меняются. Значение `name` при этом не используется
(python не нужен), реальный вызов помечается в `CallLine` как `twampy(embedded) …`.

**Зачем.** Устраняются накладные расходы на замер: запуск процесса + старт интерпретатора
python + импорт пакета. Измерено локально (замер `-c 1`, обмен с рефлектором):

| Вариант | Накладные расходы на один замер |
|---|---|
| `python -m twampy sender` (внешний процесс) | ~350 мс |
| Встроенный C# (в процессе пробы) | ~0.3–1 мс |
| Встроенный Go (в процессе пробы) | ~0–0.6 мс |

Разница в сотни раз проявляется при тысячах параллельных замеров, где спавн процесса —
узкое место. Совместимость вывода с python-версией подтверждена тестами
([`TwampySenderTests`](../SPI.Twamp.Tests/TwampySenderTests.cs) для C#,
[`twampysender_test.go`](../go-probe/twampysender_test.go) для Go): обмен с реальным
рефлектором, разбор серверным парсером, случай полной потери пакетов. Оба встроенных
отправителя сверены с настоящим python-рефлектором `nokia/twampy` — таблица идентична.

> Флаг по умолчанию **выключен** (`embedded: false`) — это экспериментальная возможность
> для проверки производительности. Внешний python-режим остаётся вариантом по умолчанию.
