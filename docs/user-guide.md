...
на одной странице с пятью вкладками. Регистрация не нужна: если на сервере включён
...
flowchart LR
    ...
    key -- нет --> tabs
    tabs["Пять вкладок"] --> t1["📡 Статус проб"]
    tabs --> t2["📋 Задачи"]
    tabs --> t3["📤 Загрузка"]
    tabs --> t5["🗄 Хранилище"]
    tabs --> t4["📥 Отчёт"]
    ...
    t5 -.- d5["следить за переносом<br/>измерений в ClickHouse"]
...
**Кнопка «состояние»** в строке подключённой пробы показывает то, что иначе видно
только в её журнале, — и именно это нужно, когда замеры вдруг перестали идти:

| Строка | О чём говорит |
|---|---|
| *Замеров идёт сейчас* | сколько замеров выполняется и каков текущий предел. Предел ниже настроенного потолка — значит проба [сжала его из-за памяти](parallelism.md#когда-запускать-проба-разгоняется-сама) |
| *Режим TWamp* / *Режим TWampy* | каким путём идут замеры: [встроенным зондом](parallelism.md#чем-различаются-пути-на-деле) (без внешнего процесса) или внешней утилитой |
| *Локальные порты зондов* | сколько портов свободно и занято. Растущее число ожиданий означает, что [пул мал](parallelism.md#как-проба-выбирает-локальные-порты) для такой нагрузки; растущее число исключённых — что диапазон [не зарезервирован в ядре](parallelism.md#что-делать-если-в-журнале-address-already-in-use) и его разбирают чужие соединения |
| *Результатов ждёт отправки* | растёт, если сервер перестал забирать результаты |
...
- в строке активной задачи — кнопка **«удалить»** (задача помечается удалённой, её можно восстановить); у удалённой — **«восстановить»** и **«стереть»** (полное удаление из БД навсегда — вторая ступень, необратимо);
...
Обе кнопки активны только когда **выбран файл маршрутизаторов и есть хотя бы один набор шаблонов**.
...
### Вкладка «Хранилище» — перенос измерений в ClickHouse

Показывает, куда и как уезжают результаты и что происходит прямо сейчас.

**Перенос измерений в ClickHouse:**

| Поле | Что означает |
|---|---|
| **Состояние** | `на связи` / `нет связи` — итог последнего обращения к базе. `выключен` — перенос отключён в настройках; `проверяется…` — обращений ещё не было |
| **Адрес базы** | адрес ClickHouse и назначение вставки (`база.таблица`) |
| **Последняя выгрузка** | когда последний сегмент успешно уехал в базу |
| **Выгружено с запуска сервера** | накопительный счётчик строк и сегментов |
| **Последняя ошибка** | текст ответа базы и время — первое, что нужно смотреть при `нет связи` |

**Буфер результатов:**

| Поле | Что означает |
|---|---|
| **Ждёт отправки** | сколько строк ещё не в ClickHouse (очередь + заполняемый сегмент) |
| **В заполняемом сегменте** | сколько строк набрано из порога `BatchRows` |
| **Очередь сегментов** | сколько сегментов ждёт отправки из предельных `MaxSegments`; полоса краснеет при переполнении |
| **Ёмкость буфера** | сколько строк система переживёт при недоступной базе |
| **Сегмент отправляется** | действующие пороги: по числу строк **или** по времени |
| **Приём с проб** | `идёт` или `приостановлен` — второе означает, что буфер полон и результаты ждут на пробах |

Что делать, если **«нет связи»**: проверьте текст последней ошибки, доступность ClickHouse
(`ClickHouse:Url`) и учётные данные. Пока база не вернулась, данные не теряются — они копятся
в буфере, а исчерпав его, остаются на пробах. После восстановления очередь уезжает сама,
приём с проб возобновляется.

Пороги переноса настраиваются — см. [конфигурацию](configuration.md#выгрузка-результатов-в-clickhouse).

---

### Вкладка «Отчёт» — выгрузка результатов

Кнопка **«Скачать CSV»** отдаёт измерения, **ещё не перенесённые в ClickHouse**. Выбора дат
здесь нет: вся история хранится в ClickHouse, и произвольный период выбирается запросом к нему
(примеры — в [Форматах файлов](file-formats.md#5-таблица-clickhouse)). Выгрузка потоковая и не
загружает сервер в память. Состав колонок — в [Форматах файлов](file-formats.md#4-отчёт-выгрузка-downloadfile).
