...
|---|---|---|
| CentOS/RHEL после `install.sh` | **32 000** (`LimitNPROC=65535`) | ничего, скрипт всё настроил |
...
fs.nr_open = 1048576
net.ipv4.ip_local_port_range = 1024 24999
```

Последняя строка — про порты, и она **разведена с `Probe:PortRange`**
(`25000-65000`). Пересекаться им нельзя, и это измерено: при совмещённых
диапазонах **30 % замеров** получают `address already in use` на законно
выданном порту, при разведённых — ни одного.

Причина в том, как ядро выбирает эфемерный порт: оно проверяет занятость и
садится ровно на **свободные** номера — те самые, что пул готовится выдать
следующему замеру. Чем плотнее занят пул, тем выше шанс попасть в оставшиеся
дырки; на боевой пробе это давало 22 % отказов.

### Бюджет портов: два номера на замер TWamp

Каждый замер TWamp занимает **два** порта:

| Что | Откуда | На что расходуется |
|---|---|---|
| UDP-порт тестового сокета | пул пробы (`Probe:PortRange`) | сами измерительные пакеты |
| TCP-порт управляющего канала | эфемерный диапазон ядра | согласование сессии twping |

Оба считаются **по каждому локальному адресу отдельно** — ядро различает
сокеты по паре «адрес + порт». Поэтому ёмкость машины:

```
одновременных замеров TWamp ≈ адресов × min(размер пула, эфемерных портов ядра)
```

Поставляемая раскладка: 24 тысячи эфемерных, 41 тысяча в пуле,
`Probe:MaxParallel = 20000` — ниже обоих, с запасом на карантин и
отлёживание. На одном адресе выше **32 тысяч** не подняться в принципе:
2 × 32000 уже не помещается в 64 512 доступных номеров.

Режима TWampy это не касается: управляющего канала у него нет, и его потолок —
только размер пула.

При старте проба сверяет всё это и предупреждает, если не сходится:

```
WARN Портов может не хватить на настроенное число замеров подробности="настроено 50000 одновременных замеров, а в пуле 41001 номеров…"
```

`ip_local_reserved_ports` для разведения не годится: он вычитает номера из
обоих пространств сразу, то есть отнимет их и у управляющих соединений twping.
...
по умолчанию **25000–65000**.

### Почему именно этот диапазон

Он **разведён** с эфемерным диапазоном ядра (`1024 24999` в поставляемом
`99-twamp-probe.conf`), и это главное: пересечение стоит трети замеров, что
измерено (см. [бюджет портов](#бюджет-портов-два-номера-на-замер-twamp)).

Размер выбран так, чтобы ядру осталось на управляющие соединения twping —
их нужно столько же, сколько замеров. 41 тысяча номеров в пуле против
24 тысяч у ядра: пул заведомо не станет узким местом.

### Номера считаются по каждому адресу отдельно

Ядро различает сокеты по паре «адрес + порт»: `10.0.0.1:30000` и
`10.0.0.2:30000` — два разных сокета, а не конфликт. Поэтому пул ведёт свой
набор номеров на каждый локальный адрес, и ёмкость машины умножается на их
число:

```
INFO Порты зондов раздаёт проба — общий пул для TWamp и TWampy диапазон=25000-65000 на_адрес=41001 адресов=3 всего=123003
```

Адрес пробе не безразличен: она определяет его до выдачи номера — тем же
способом, что и ядро (маршрут до узла), — и передаёт зонду явно: `-S` у
twping, `адрес:порт` в near-end у twampy. Без этого зонд встал бы на
`0.0.0.0`, заняв номер сразу на всех адресах, и учёт разошёлся бы
с действительностью.

Общий на всю машину счёт номеров отнимал бы у пробы ровно во столько раз
больше ёмкости, сколько на ней адресов.
...
### Когда зонд не отпускает порт

Аренда заканчивается, когда зонд закрыл сокет. У встроенного twampy это всегда
так: его цикл сам проверяет дедлайн и отмену и закрывает сокет, выходя.

С twping так было не всегда. До версии библиотеки 1.2.0 временем было
ограничено только **подключение** (30 секунд), а дальше — чтение приветствия и
весь обмен по управляющему каналу — шло без дедлайнов и на отмену не смотрело.
Достаточно было рефлектору принять TCP-соединение и замолчать, и замер повисал
навсегда: ни таймаут задачи, ни её удаление его не снимали.

Чем это кончается, видно по боевой пробе: замеры копятся, задачи вечно
числятся выполняющимися (а значит, их новые запуски **пропускаются**), их порты
остаются в аренде — и пул исчерпывается. На пробе, где такое случилось,
в аренде числилось 46 тысяч номеров при 3169 задачах в журнале.

Вылечено в [twping-go 1.2.0](https://github.com/akprof2000/twping-go): у каждого
шага управляющего обмена свой дедлайн (`ControlConfig.ExchangeTimeout`, по
умолчанию 30 секунд), а отмена контекста закрывает соединение — дедлайн спасает
от молчания, закрытие от отмены посреди уже начатого чтения.

Проба вдобавок сторожит клиента сама: ждёт ровно столько, сколько отпущено
задаче, и даёт секунду на то, чтобы свернуться. Если и после этого он держит
сокеты, порт в пул **не возвращается** — он занят, — а уходит в карантин:

```
WARN Зонд не остановился по таймауту — порт остаётся занятым порт=31234 адрес=10.0.0.9 режим=TWamp …
```

Строка эта означает, что клиент повёл себя не так, как ожидалось; при исправной
библиотеке её быть не должно.
...
Всё, что касается портов, достоверно только на Linux: в Windows два UDP-сокета
уживаются на одном номере, свой эфемерный диапазон и нет `/proc/net/udp`.
Поэтому набор гоняется в контейнере с той же системой, где работает проба:

```bash
run-tests-centos.cmd                             # весь набор
run-tests-centos.cmd -test.run TestLoad_ -test.v # только нагрузочные
```

Нагрузочные (`TestLoad_`, файл `portlease_linux_test.go`) меняют
`ip_local_port_range`, поэтому требуют `--privileged` — настройка сетевая,
правится в своём пространстве имён и хозяйскую машину не задевает. В обычном
прогоне и в CI они пропускаются.

Что они показывают:

| Проверка | Результат |
|---|---|
| один опыт при совмещённых и разведённых диапазонах | **29.8 %** столкновений против **0 %** |
| владелец порта в момент отказа (`/proc/net/udp`) | `0.0.0.0:20878` — сокет на всех адресах |
| пул выдал номер двум арендаторам | **0 раз** на 6000 аренд с настоящей привязкой сокетов |
| сокеты в диапазоне пула, не числящиеся в аренде | **0** (5 срезов посреди 300 одновременных замеров) |
| один номер на разных адресах | занимается одновременно, ёмкость = диапазон × адреса |
| 400 одновременных встроенных twping | 0 неудач, аренда вернулась целиком |
...
[выше](#что-настраивает-installsh)).
...
1. **Поднять лимиты системы** — `LimitNPROC` в юните и `sysctl` из
   `install.sh`. Предупреждение при старте пробы говорит, какой именно
