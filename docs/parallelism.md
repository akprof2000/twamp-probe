...
С twping сложнее. Библиотека ограничивает временем только **подключение**
(30 секунд), а дальше — чтение приветствия и весь обмен по управляющему
каналу — идёт без дедлайнов и на отмену не смотрит. Достаточно рефлектору
принять TCP-соединение и замолчать, и замер повисает навсегда: ни таймаут
задачи, ни её удаление его не снимают.
...
Поэтому проба ждёт клиента ровно столько, сколько отпущено задаче, и
возвращается сама. Порт такого замера в пул **не возвращается** — он занят,
пока клиент внутри не закончит, — а уходит в карантин и вернётся в оборот
позже:
...
Настоящее лечение — дедлайны на управляющем канале в самой библиотеке
[twping-go](https://github.com/akprof2000/twping-go); проба защищает себя тем,
что ей доступно.
