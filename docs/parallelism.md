...
flowchart TD
    ...
    busy -->|да| ban["Порт исключается:<br/>его занял посторонний"]
...
4. **Занят — берём другой.** Если ядро всё же ответило `address already in use`,
   порт исключается из оборота, а замер **повторяется с другим номером** (до
   десяти попыток). Раньше такой замер просто пропадал:

```
WARN Порт занят посторонним процессом — исключён из пула, пробуем другой порт=20134 попытка=1 …
```
...
порт в сообщении и есть ли рядом строка про исключение из пула.
...
и **исключено**. Растущее число исключённых — верный признак, что резервирование
не применено.
