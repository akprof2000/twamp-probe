...
flowchart TD
    ...
    ban --> again{"Попыток<br/>меньше трёх?"}
    again -->|да| check
...
4. **Занят — берём другой.** Если ядро всё же ответило `address already in use`,
   порт исключается из оборота, а замер **повторяется с другим номером** (до трёх
...
```
   ...
   Три попытки — предел не от осторожности: если заняты и они, дело не в
   невезении, а в том, что пул пересекается с чужими соединениями. Это лечится
   настройкой, а не повторами (см. ниже).
