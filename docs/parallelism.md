...
Последняя строка — про порты, и она работает в паре с `Probe:PortRange`
(`9000-65000`): **диапазоны разведены по построению**, ядру отдан участок ниже
пула. Пересечься они не могут, поэтому чужое соединение не встанет на номер,
законно выданный пулом, и `ip_local_reserved_ports` не нужен — резервировать
пул на 56 тысяч портов было бы нечем, ядру не осталось бы ничего.

Обратная сторона: у ядра остаётся около 8 тысяч портов на исходящие
соединения, а одно такое нужно каждому замеру TWamp — управляющий канал twping
идёт по TCP. Это и есть потолок: примерно **8 тысяч одновременных замеров
TWamp**. Нужно больше — сдвиньте нижнюю границу пула вверх (например
`16000-65000`) и расширьте диапазон ядра до `1024 15999`. Режима TWampy это не
касается: у него управляющего соединения нет вовсе.
...
Он **разведён с эфемерным диапазоном ядра**: поставляемый
`99-twamp-probe.conf` оставляет ядру `1024-8999`, всё что выше принадлежит
пулу. Пересечься они не могут, значит наш порт не окажется внезапно занят
чужим исходящим соединением. Побочная выгода: давление на эфемерный диапазон
падает вдвое — из него берётся только управляющее TCP-соединение, а тестовый
UDP-сокет идёт из нашего пула.

Размер выбран с запасом: 56 тысяч портов — это потолок в десятки раз выше
любой реальной нагрузки, поэтому замерам не приходится ждать освобождения,
а номера успевают отлежаться между использованиями.
...
Потому что ядро раздаёт эфемерные порты из того же участка, откуда пул выдаёт
свои: чужое исходящее соединение встаёт на номер, законно выданный замеру.

Лечится это разведением диапазонов, и в поставляемых настройках они уже
разведены: пул занимает `9000-65000`, ядру отдано всё, что ниже
(`net.ipv4.ip_local_port_range = 1024 8999`). Пересечься они не могут.

**Меняете `Probe:PortRange` — меняйте и настройку ядра.** Если диапазоны
наложатся, проба скажет об этом при старте (`WARN Диапазон портов пересекается
с эфемерным диапазоном ядра`), а в журнале пойдут `address already in use`
на законно выданных портах.
...
Второй способ развести — не сужать диапазон ядра, а вычесть пул из него
списком `net.ipv4.ip_local_reserved_ports`. Он годится для небольшого пула
(так было раньше: пул `20000-32767` резервировался списком, и проверка
показала, что 15 000 подряд открытых эфемерных сокетов не заняли ни одного
нашего номера). Для пула на 56 тысяч портов этот путь не работает: вычесть
его целиком — значит не оставить ядру ничего.
...
Но для режима TWamp раньше упрётесь в другое: каждому замеру нужно ещё и
исходящее TCP-соединение (управляющий канал twping), а их ядро берёт из своих
`1024-8999` — около **8 тысяч**. Нужно больше: сдвиньте нижнюю границу пула
вверх и расширьте диапазон ядра, например
...
Режима TWampy это не касается — управляющего соединения у него нет, и потолком
остаётся размер пула.
