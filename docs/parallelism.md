...
4. **Исключение занятых.** Если ядро всё же ответило `address already in use`,
   порт занят посторонним процессом — он исключается из оборота и больше не
   выдаётся. Иначе каждый следующий замер наступал бы на те же грабли:

```
WARN Порт занят посторонним процессом — исключён из пула порт=20134 задача=… узел=…
```
