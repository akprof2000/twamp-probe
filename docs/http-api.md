...
Действия, которые обращаются к пробе (`checkin`, `probetaskstatus`, `probestate`), при её недоступности отвечают текстом с причиной: `502` — соединение не удалось («Проба … недоступна: Connection refused»), `504` — проба не ответила за `Probe:HttpTimeoutSec`. Голого `500` в этих случаях больше нет.
...
| `portsAddresses` | локальных адресов в обороте — номера считаются по каждому отдельно |
| `portsCapacity` | всего номеров: диапазон × число адресов |
...
| `portsBanned` | в карантине: заняты посторонним процессом либо зондом, не отпустившим порт |
