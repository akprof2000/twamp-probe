...
# Хелперы и константы вынесены в twampy.core, чтобы их разделяли и классические
# одиночные режимы (этот модуль), и масштабируемый флот (twampy.fleet).
from twampy.core import (
    ALLBITS,
    TIMEOFFSET,
    dp,
    log,
...
from twampy.fleet import responder_fleet, sender_fleet, spawn
...
def main():
    ...
    # Форк использует русскоязычный вывод. Чтобы редкий несопоставимый символ
    # (напр. стрелка) не ронял инструмент на консоли с кодировкой без него,
    # переключаем обработчик ошибок кодирования на "replace" (кодировку самой
    # консоли сохраняем — кириллица в cp1251/cp866 кодируется корректно).
    ...
    # Порт 0 = эфемерный: ОС сама выдаёт свободный UDP-порт, поэтому тысячи
    # параллельных отправителей не конфликтуют за один локальный порт (:20000).
    group.add_argument("near_end", nargs="?", metavar="local-ip:port", default=":0")
    ...
    # ------------------------------------------------------------------ #
    # Доработка twampy-fleet: масштабируемые режимы (до ~10000 экземпляров)
    # ------------------------------------------------------------------ #

    # Флот рефлекторов: один процесс биндит N последовательных портов на asyncio
    p_rfleet = subparsers.add_parser(
        "responder-fleet",
        help="флот TWL рефлекторов на N портах (один процесс, asyncio)",
        parents=[debug_parser, ipopt_parser],
    )
    group = p_rfleet.add_argument_group("responder-fleet options")
    group.add_argument("near_end", nargs="?", metavar="local-ip:base-port", default=":20001")
    group.add_argument("-n", "--count", metavar="N", default=100, type=int, help="число рефлекторов (портов)")
    group.add_argument("--step", metavar="delta", default=1, type=int, help="шаг между портами (по умолчанию 1)")
    group.add_argument("--report", metavar="sec", default=10, type=int, help="период вывода статистики, сек")

    # Флот отправителей: N отправителей с последовательных исходных портов
    p_sfleet = subparsers.add_parser(
        "sender-fleet",
        help="флот TWL отправителей (один процесс, asyncio)",
        parents=[debug_parser, ipopt_parser],
    )
    group = p_sfleet.add_argument_group("sender-fleet options")
    group.add_argument("far_end", nargs="?", metavar="remote-ip:base-port", default="127.0.0.1:20001")
    group.add_argument("near_end", nargs="?", metavar="local-ip:base-port", default=":20000")
    group.add_argument("-n", "--count", metavar="N", default=100, type=int, help="число отправителей")
    group.add_argument("--step", metavar="delta", default=1, type=int, help="шаг между исходными портами")
    group.add_argument(
        "--target-step",
        metavar="delta",
        default=1,
        type=int,
        help="шаг между целевыми портами (0 = все в один порт)",
    )
    group.add_argument("-c", "--packets", metavar="packets", default=100, type=int, help="пакетов на отправителя")
    group.add_argument("-i", "--interval", metavar="msec", default=100, type=int, help="интервал отправки, мс")

    # Лаунчер отдельных ОС-процессов
    p_spawn = subparsers.add_parser(
        "spawn",
        help="запустить несколько отдельных процессов флота, поделив диапазон портов",
        parents=[debug_parser, ipopt_parser],
    )
    group = p_spawn.add_argument_group("spawn options")
    group.add_argument("--mode", choices=["responder", "sender"], required=True, help="какой флот запускать")
    group.add_argument("--procs", metavar="K", default=4, type=int, help="число процессов")
    group.add_argument("--host", metavar="ip", default="", help="локальный IP для привязки (по умолчанию все)")
    group.add_argument("--base-port", metavar="port", default=20001, type=int, help="начальный исходный порт")
    group.add_argument("--count", metavar="N", default=100, type=int, help="всего экземпляров на все процессы")
    group.add_argument("--step", metavar="delta", default=1, type=int, help="шаг между портами")
    # только для sender:
    group.add_argument("--target-host", metavar="ip", default="127.0.0.1", help="[sender] IP мишени")
    group.add_argument("--target-port", metavar="port", default=20001, type=int, help="[sender] начальный порт мишени")
    group.add_argument("--target-step", metavar="delta", default=1, type=int, help="[sender] шаг портов мишени")
    group.add_argument("--packets", metavar="packets", default=100, type=int, help="[sender] пакетов на отправителя")
    group.add_argument("--interval", metavar="msec", default=100, type=int, help="[sender] интервал отправки, мс")
    ...
    p_rfleet.set_defaults(parseop=True, func=responder_fleet)
    p_sfleet.set_defaults(parseop=True, func=sender_fleet)
    p_spawn.set_defaults(parseop=True, func=spawn)
