...
// настройкой ядра net.ipv4.ip_local_port_range из 99-twamp-probe.conf: ядру
// отдан участок ниже 25000, пулу — всё, что выше. Пересекаться им нельзя —
// ядро выбирает эфемерный порт среди свободных, то есть садится ровно на те
// номера, что пул готовится выдать (проверено: при совмещённых диапазонах
// 30 % замеров получают «address already in use»).
//
// Номера считаются по каждому локальному адресу отдельно, поэтому ёмкость —
// это размер диапазона, умноженный на число адресов машины.
const defaultPortRange = "25000-65000"
