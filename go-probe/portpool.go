...
//
// Диапазон по умолчанию (9000–65000) разведён с эфемерным диапазоном ядра:
// поставляемый 99-twamp-probe.conf оставляет ядру всё, что ниже 9000. Оттуда
// ядро порты не раздаёт, поэтому столкнуться с чужим сокетом почти невозможно.
// Если порт всё же занят посторонним процессом, зонд сообщит об этом, и пул
// уведёт порт в карантин — см. Blacklist.
//
