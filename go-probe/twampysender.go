...
func runEmbeddedTwampy(args []string, deadline time.Time) (output, errText string) {
	opts, err := parseTwampyArgs(args)
	if err != nil {
		return "", fmt.Sprintf("Некорректные аргументы twampy sender: %v", err)
	}

	table, runErr := twampySession(opts, deadline)
	if runErr != nil {
...
func twampySession(opts *twampyOptions, deadline time.Time) (string, error) {
	network := "udp4"
